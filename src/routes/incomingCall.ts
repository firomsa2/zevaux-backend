...
//     // create session
//     const session = new CallSession({ callSid, from, to });
//     try {
//       await session.loadBusinessAndConfig();
//       await session.createCallRow({ twilio: params });
//     } catch (err: any) {
//       log.error("incoming call setup error", err?.message || err);
//       const errorTwiML = `<?xml version="1.0" encoding="UTF-8"?><Response><Say>We're sorry, can't connect right now.</Say></Response>`;
...
//     log.info("Generated TwiML with WebSocket URL:", wsUrl);
//     reply.type("text/xml").send(twiml);
//   });
// }

// routes/incomingCall.ts
import { FastifyInstance } from "fastify";
import { CallSession } from "../models/CallSession.js";
import { SessionManager } from "../core/sessionManager.js";
import { signCallToken } from "../utils/hmac.js";
